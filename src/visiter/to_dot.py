...
def main():
    """CLI: pass to_dot's keyword arguments as a single Python expression.

    Graph input is read from stdin (or --input FILE). The argument string is
    spliced into `to_dot(graph, <argstring>)` and eval'd. DOT source is
    written to stdout (or -o FILE).

    Example:
        visiter iterate '...' | visiter to-dot 'anchor=1, radius=17'
    """
    import argparse
    import json
    import sys
    ...
    parser = argparse.ArgumentParser(
        description="Render an iteration graph (JSON on stdin) as Graphviz DOT.",
    )
    parser.add_argument("argstring", nargs="?", default="",
                        help="Python kwargs spliced into to_dot(graph, <ARG>)")
    ...
    call = (f"to_dot(graph, {args.argstring})"
            if args.argstring.strip() else "to_dot(graph)")
    dot = eval(call, ns)
